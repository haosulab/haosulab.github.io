...
def create_cartpole(
    ...
    cart.add_box_shape(half_size=cart_half_size, density=0.01)
    ...
    rod.add_box_shape(half_size=rod_half_size, density=0.0001)
...
def main():
    ...
    Kp = np.array([10, 0])*1e-2
    Kd = np.array([0, 0])*1e-2
    Ki = np.array([0, 0])*1e-2
    lmd = 0.01
    ...
    while not viewer.closed:
        ...
        e = np.array([pos-p_d, theta-th_d])
        dedt = np.array([vel-dpdt_d, omega-dthdt_d])
        ie += e

        f = np.array([0])
        if steps >= perturb_round:
            E = 0.5*(M+m)*(vel*vel)-m*l*vel*omega*cos(theta)+0.5*m*l*l*omega*omega+m*gravity*l*cos(theta)
            Ed = m*gravity*l
            Ediff = E-Ed

            ff = -k*vel*Ediff
            fd = -lmd*(Kp.T@e+Kd.T@dedt+Ki.T@ie)
            f = ff + fd
            print('e', e, 'dedt', dedt)
            print('controlling, at step %d.' % (steps-perturb_round), 'Ediff', Ediff, 'ff', ff, 'fd', fd, 'ftot', ff+fd)
            cartpole.set_qf(np.array([f, 0, 0]))
        else:
            print('perturbing the system')
            cartpole.set_qf(np.array([0, 1, 0]))
        ...
        if steps % 10 == 0:
            ...
            if steps <= perturb_round:
                ...
                draw.text((0, 0), 'controlling the system, at step %d, f=%f' % (steps-perturb_round, f), (0,0,0), font=font)
